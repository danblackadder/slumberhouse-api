...
import { GroupRole, OrganizationRole, UserStatus } from '../../types';
...
export const createUser = async ({
  ...
  role = [OrganizationRole.ADMIN, OrganizationRole.BASIC].sort(() => 0.5 - Math.random()).slice(0, 1)[0],
...
export const createGroups = async ({
  ...
  for (let i = 0; i < count; i++) {
    ...
    await GroupUsers.create({ role: GroupRole.ADMIN, groupId: group._id, userId });
